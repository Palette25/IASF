...
def generate_ptbxl_summary_table(selection=None, folder='../output/'):
    ...
    # get models（用 os.path.basename 兼容 Windows 下 glob 返回反斜杠路径）
    models = {}
    for i, exp in enumerate(exps):
        if selection is None:
            exp_models = [os.path.basename(m.rstrip(os.sep)) for m in glob.glob(os.path.join(folder, str(exp), 'models', '*'))]
        else:
    ...
    for m in models:
        ...
        for e in exps:
            ...
            try:
                me_res = pd.read_csv(os.path.join(folder, str(e), 'models', str(m), 'results', 'te_results.csv'), index_col=0)
...
def ICBEBE_table(selection=None, folder='../output/'):
    ...
    if selection is None:
        models = [os.path.basename(m.rstrip(os.sep)).split('_pretrained')[0] for m in glob.glob(os.path.join(folder, 'exp_ICBEB', 'models', '*'))]
    else:
        models = [] 
        for s in selection:
            #if s != 'Wavelet+NN':
    ...
    data = []
    for model in models:
        me_res = pd.read_csv(os.path.join(folder, 'exp_ICBEB', 'models', model, 'results', 'te_results.csv'), index_col=0)
        mcol=[]
        for col in cols:
