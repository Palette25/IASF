    ...
    def evaluate(self, n_bootstraping_samples=100, n_jobs=20, bootstrap_eval=False, dumped_bootstraps=True):
        ...
        if bootstrap_eval:
            if not dumped_bootstraps:
                ...
                train_samples = np.load(self.outputfolder+self.experiment_name+'/train_bootstrap_ids.npy', allow_pickle=True)
        ...
        for m in sorted(os.listdir(self.outputfolder+self.experiment_name+'/models')):
            ...
            # 为了兼容 Windows，统一使用单进程循环方式计算 bootstrap 统计量，避免 multiprocessing spawn 问题
            def _eval_split(samples, y_true, y_pred):
                # samples: array of index arrays; y_true/y_pred: full arrays
                dfs = [utils.generate_results(idx, y_true, y_pred, thresholds) for idx in samples]
                df = pd.concat(dfs)
                df_point = utils.generate_results(range(len(y_true)), y_true, y_pred, thresholds)
                df_result = pd.DataFrame(
                    np.array([
                        df_point.mean().values,
                        df.mean().values,
            ...
            tr_df_result = _eval_split(train_samples, y_train, y_train_pred)
            val_df_result = _eval_split(val_samples, y_val, y_val_pred)
            te_df_result = _eval_split(test_samples, y_test, y_test_pred)
